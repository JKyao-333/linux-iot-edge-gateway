...
项目根目录的 `VERSION` 是唯一版本来源，格式必须为 `MAJOR.MINOR.PATCH`，例如 `1.0.1`。
...
`./scripts/verify_release_version.sh v1.0.1`
...
- `linux-iot-edge-gateway-1.0.1-aarch64.tar.gz`
- `linux-iot-edge-gateway-1.0.1-aarch64.tar.gz.sha256`
...
3. 提交版本变更并推送 `main`。
4. 创建与版本一致的标签，例如 `git tag -a v1.0.1 -m "release v1.0.1"`。
5. 推送标签，例如 `git push origin v1.0.1`。
...
`sha256sum -c linux-iot-edge-gateway-1.0.1-aarch64.tar.gz.sha256`

GitHub Release 工作流还会通过官方 `actions/attest-build-provenance` 生成构建来源证明。该证明绑定仓库、工作流、提交和产物摘要，可使用 GitHub CLI 验证：

`gh attestation verify linux-iot-edge-gateway-1.0.1-aarch64.tar.gz --repo JKyao-333/linux-iot-edge-gateway`
