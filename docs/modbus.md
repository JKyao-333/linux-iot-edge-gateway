...
当前实现支持单个配置从站的 FC03/FC04 周期读取、响应校验、异常响应识别和参考寄存器映射。Modbus 可以作为唯一输入源运行，也可以与 UART、SocketCAN 同时启用。
...
当前实现是单请求串行轮询模型，不支持写线圈/写寄存器、多从站调度、广播、自动设备发现或设备厂商寄存器模板。参考映射不能直接替代真实设备手册。socat/PTTY 测试验证软件行为，不等同于 RS485 总线电气特性、终端电阻、EMC 或现场布线验证。
