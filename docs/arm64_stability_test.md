...
项目已在 ARM64 Linux 开发板上完成串口接收、MQTT/TCP 上报、离线缓存补传和异常恢复的稳定性验证。本文件记录仓库可公开的测试边界与复现方法。

原始设备台账和监控时序数据未随仓库提交，因此开发板完整型号、系统镜像摘要、精确运行时长以及 CPU/内存统计值需要从课题组实验记录补录。外部项目只用于选择可复现的参考硬件，不用于填充本项目实测结果。
...
复现实验可参考 `smartmeter2mqtt` 公开的 Raspberry Pi 4 64-bit ARM64 与稳定串口设备路径方案。推荐参考板为 Raspberry Pi 4 Model B + 64 位 Linux；该组合不是本项目历史实测板卡型号的替代证据。
...
ARM64 Linux 板级环境下的核心数据链路和故障恢复路径已完成稳定性验证。仓库中的交叉编译、QEMU 启动、CI、Python 模拟器和 socat 测试提供可重复的软件回归路径；板卡电气特性、内核驱动和长时间资源统计仍以课题组原始实验台账为准。
